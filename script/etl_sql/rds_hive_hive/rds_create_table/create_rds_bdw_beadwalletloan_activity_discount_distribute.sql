drop table if exists rds.rds_bdw_beadwalletloan_activity_discount_distribute ;
create table rds.rds_bdw_beadwalletloan_activity_discount_distribute (`distribute_id` bigint comment '主键',
...
`amount` decimal(38,6) comment '优惠金额',
`loan_amount` decimal(38,6) comment '限定金额',
...
`update_time` string comment '优惠券操作人' )comment '活动优惠派发表'partitioned by (is_valid int,ce_start_date int,ce_end_date int) stored as parquet;