...
def auto_sql(df_columns,df_tables, type_dict,column_source,target):
    ...
    for table_name_inial in set(df[2]):
        ...
        for table_schema in range(len(temp2[0])):
            ...
            try:
                sql = ods_alter_sql(temp, table_name_new, type_dict, column_source,target,dbs,table_name_inial)
                filepath = "../../script/etl_sql/ods_mysql_hive/ods_alter_table"
...
def ods_alter_sql(temp, table_name_all, type_dict, column_source,target,dbs,table_name_inial):
    ...
    for i in list(temp[4]):
        if temp[temp[4] == i][11].values[0] == 1 :
            ...
            change_count+= 1
            change_lis.append('''ALTER TABLE {}  change `{}` `{}` {} comment '{}' cascade;'''.format(table_name_all,i.lower(),i.lower(), type_conversion(temp[temp[4] == i][5].values[0] if len(temp[temp[4] == i][5]) !=0 else print("Error: table:{} column_type is None".format(table_name_all)), type_dict, column_source,target),temp[temp[4]==i][9].values[0]))
        else:
            pass
    ...
    if add_count!=0 and change_count!=0 :
        sql = "use ods;{alter_add}{change_lis}".format(alter_add=alter_add+';\n',change_lis= change_lis)
    elif add_count!=0 and change_count==0 :
        sql = "use ods;{alter_add}".format(alter_add=alter_add+';\n' )
        print("WARN:dbs:ods_{} table:{} chang is None".format(dbs,table_name_inial))
    elif add_count==0 and change_count!=0 :
        sql = "use ods;{change_lis}".format(change_lis= change_lis)
        print("WARN:dbs:ods_{} table:{} add is None".format(dbs,table_name_inial))
    else :
        print("WARN:dbs:ods_{} table:{} chang is None".format(dbs,table_name_inial))
        print("WARN:dbs:ods_{} table:{} add is None".format(dbs,table_name_inial))
    # print(sql)
